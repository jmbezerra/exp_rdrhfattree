...
class NetworkMonitor(app_manager.RyuApp):
	...
	def _demandEstimator(self, flows, hostsList):
		...
		for flow in flows:
			if flow['demand'] > 0.1:
				lista.append(flow)
		file = open("./TCC/Algoritmos/flows", "w")
		file.write(str(len(lista)) + "\n")
		for flow in lista:
			file.write(str(flow['src']) + " " + str(flow['dst']) + " " + str(flow['demand']) + "\n")
		file.close()
		os.system("./TCC/Algoritmos/compile.sh")
		os.system("./TCC/Algoritmos/run.sh")
		file = open("./TCC/Algoritmos/results.txt", 'r')
		n = int(file.readline()[:-1])
		for i in range(n):
			m = int(file.readline()[:-1])
			line = list(map(int, file.readline()[:-1].split()))
			self._install_GFF_path(line, lista[i]['match'], lista[i]['priority'])
		file.close()
